...
import type { StudentActionState, StudentFormValues } from "@/lib/validators/student";
import { studentFormSchema } from "@/lib/validators/student";

function getValidationState(values: StudentFormValues):
...
export async function createStudent(values: StudentFormValues): Promise<StudentActionState> {
...
export async function updateStudent(id: number, values: StudentFormValues): Promise<StudentActionState> {
	const validated = getValidationState(values);
	if (!validated.success) {
		return validated.state;
	}
